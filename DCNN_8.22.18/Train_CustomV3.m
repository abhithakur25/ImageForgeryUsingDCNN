%% Train_CustomV3.m
% Phase 2 (iteration v3): same deeper CNN as v2 but with STABILISED optimization
% to fix v2 getting stuck at the random-guess plateau (val loss ~0.693).
% Changes vs v2: lower initial LR (3e-4), gradient clipping, smaller FC head,
% warmup-free Adam. Produces the full metric suite + loss/accuracy curves.
%
% Run: matlab -batch "VARIANT='v3'; Train_CustomV3"

clc; close all;
if ~exist('VARIANT','var') || isempty(VARIANT); VARIANT = 'v3'; end
...
layers = [
    ...
    fullyConnectedLayer(128,'Name','fc1'); reluLayer('Name','relu_fc')
